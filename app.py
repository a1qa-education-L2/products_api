...
parser.add_argument('price', type=float, location='args')
...
class Product(Resource):
    ...
    def post(self):
        ...
        if not (args["name"] and args["type"] and args["price"]):
        ...
        if args["type"] not in config.ALLOWED_PRODUCT_TYPES:
            return {"message": "product type '{}' is not applicable".format(args["type"])}, 400
        if args["price"] and not 0.99 <= args["price"] <= 5000:
            return {"message": "product price is out range (0,99 to 5000)"}, 400
        if args["rating"] and not 0 <= args["rating"] <= 5:
            return {"message": "product rating is out range (0 to 5)"}, 400
        if args["num"] and not 0 <= args["num"] <= 1000:
...
class ProductNumber(Resource):
    def get(self, product_id=None):
        ...
        if sum(bool(i) for i in (args["id"], args["type"], args["rating"])) > 1:
            return {"message": "the query should contain only one required parameter ('id', 'type', 'rating')"}, 400
